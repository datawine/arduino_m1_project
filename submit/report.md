...
周京汉：这个实验是在小实验的基础上做的一个大拓展开发，在技术层面上其并不是一个很大的技术难关，我认为其难点更多的存在与如何利用一个更好的系统去能够流畅，安全的完成其所需要的全部功能。这即涉及硬件的问题（能流畅的完成其硬件功能），也涉及软件工程方面的知识（让整个系统鲁棒的运行起来）。这种综合考虑的需求让我们在完成大实验的过程中对多方面知识有了更加深刻的了解。